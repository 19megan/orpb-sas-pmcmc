...
class ModelInterfaceMesas:
    ...
    def _update_df_scale(self, scale_param: dict) -> None:
        ...
        for flux in scale_param.keys():
            ...
            delta_S = self.df["storage (mm)"].values #dynamic storage
    ...
    def _init_sas_model(self) -> None:
        ...
        self.model = self.model_type(
            self.df,
            config={
                "sas_specs": deepcopy(self.sas_specs), #sas_specs,
                "solute_parameters": deepcopy(self.solute_parameters), #solute_parameters,
                "options": deepcopy(self.options), #options,
            },
            verbose=False,
        )
