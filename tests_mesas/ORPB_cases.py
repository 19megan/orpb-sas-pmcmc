...
sas_specs_storage_q_ug_et_u = {
        ...
        {'quickflow (mm/hr) SAS function':
            {'func':'kumaraswamy', #uniform dist
             'args': {
                 ...
                 'b': 1.0,
                 'loc': 0.0,
                 'scale': 0.254 #just estimating scale when it's not a string
                 }}
    ...
    'baseflow 1 (mm/hr)':
        {'baseflow 1 (mm/hr) SAS function':
                {'func': 'gamma',
                 'args': { 'a': 1.26, # not exponential bc a != 1, so we estimate 'a'
                           'loc': 0,
                           'scale':'storage (mm)' #2106.1584 # string so we estimate lamda and S_c
                            }}
        },
    'ET (mm/hr)': 
...
obs_uncertainty = {
    ...
    'sigma C out': { #ORPB 18O':{
        'prior_dis': 'normal',
        'prior_params':[0.02, .02], #[5.0, 5.0], #[-7.325, 0.581], # mean, std
        'is_nonnegative': True #False
    }
}                  
